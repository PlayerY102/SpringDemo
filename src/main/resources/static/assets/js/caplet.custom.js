$(function () {
    ...
    //////////     WIDGET IM      //////////
    $("a.im-action").click(function () {
        imClose($(this), $(this).closest('section'));
        $(this).closest('section').toggleClass("push-in");
    });
    $("a.im-delete").click(function () {
        imClose($(this), $(this).closest('section'));
        $(this).closest('section').toggleClass("push-in");

        $(this).parents('li').attr('id','delNow');

        console.log($(this).parents('li'));
    });
    $("a.im-accept").click(function () {
        imClose($(this), $(this).closest('section'));
        $(this).closest('section').toggleClass("push-in");
        var parent=$(this).parents('li');

        $(this).parents('li').attr('id','acceptNow');
    ...
    $("a.im-confirm").click(function () {
        ...
        var confirmClose = function () {
            parents.fadeOut(400, function () {
                var data = "messageId=" + this.value;

                console.log(this);

                if (this.id == "acceptNow") {
                    $.ajax({
                        type: "POST",
                        url: "/addTransaction",
                        data: data,
                ...
                },
                {
                    series: {
                        ...
                        pie: {
                            ...
                            stroke: {shadow: data.pieStyle == "shadow" ? true : false},
                            label: {show: data.pieLabel}
                        },
                        bars: {show: bars, barWidth: data.barWidth || 0.5, fill: data.fill || 1, align: "center"},
                        lines: {show: lines, fill: 0.1, steps: data.steps}
                    },
